...
;; must return t.  Call `svg-clock-timer-toggle' to start/stop the clock.
...
(defun svg-clock-timer-start ()
  "Start the svg clock timer."
  (if svg-clock-timer
    ...
    (message "svg clock starts.")))

(defun svg-clock-timer-stop ()
  "Stop the svg clock timer."
  (interactive)
  (if (not svg-clock-timer)
      (message "svg clock is not running.")
    (cancel-timer svg-clock-timer)
    (setq svg-clock-timer nil)
    (message "svg clock stops.")))

(defun svg-clock-timer-toggle ()
  "Toggle start/stop the svg-clock."
  (interactive)
  (switch-to-buffer (get-buffer-create "*clock*"))
  (if svg-clock-timer
      (svg-clock-timer-stop)
    (svg-clock-timer-start)))

(defun svg-clock-quit ()
...
(defvar svg-clock-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map [?q] 'svg-clock-quit)
    (define-key map [?s] 'svg-clock-quit)
    (define-key map [?s] 'svg-clock-timer-toggle)
    (define-key map [?f] 'svg-clock-fit-window)
    (define-key map [?+] 'svg-clock-grow)
    (define-key map [?-] 'svg-clock-shrink)
    map))

(define-derived-mode svg-clock-mode image-mode "svg clock"
...
;;;###autoload
(defun svg-clock ()
  "Create *clock* buffer and start the svg clock timer."
  (interactive)
  (switch-to-buffer (get-buffer-create "*clock*"))
  (svg-clock-mode)
  (svg-clock-timer-start))

...
(define-key top-mode-map "e" 'top-auto-update-stop)
(define-key top-mode-map "b" 'top-auto-update-start)
...
(define-key top-mode-map "q" 'top-quit)
...
(defvar top-auto-update nil)

(defun top-auto-update-start ()
  "Start *top* auto update"
  (interactive)
  (if top-auto-update
      (message "*top* auto-update is running already")
    (switch-to-buffer (get-buffer-create "*top*"))
    (setq top-auto-update
          (run-with-timer 0 3 'top-update))
    (message "*top* auto-update started")))

(defun top-auto-update-stop ()
  "Stop *top* auto update"
  (interactive)
  (if (not top-auto-update)
      (message "*top* auto-update is not running.")
    (cancel-timer top-auto-update)
    (setq top-auto-update nil)
    (message "*top* auto-update stopped")))
...
(defun top-mode-fill-buffer (go-top)
  (switch-to-buffer (get-buffer-create "*top*"))
  (if (top-update go-top)
      (top-mode)))

(defun top-update (&optional go-top)
  (with-current-buffer (get-buffer-create "*top*")
...
(defun top-mode-on-pid-line ()
  (when (save-excursion
          ...
          (let ((orig-line (line-number-at-pos)))
            (while (and (equal (line-number-at-pos) orig-line)
                        (looking-at "\\s-"))
              (forward-char 1))
            (and (equal (line-number-at-pos) orig-line)
                 (looking-at "[0-9]+\\s-"))))

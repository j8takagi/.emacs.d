...
;; user-emacs-directory(~/.emacs.d)のサブディレクトリーをload-pathに追加
(unless noninteractive
...
(listify-requires
 ...
 'window-control                   ; ウィンドウとフレームのサイズを調整
 'xlfd-at                          ; フォント情報の表示
 ;; ~/.emacs.d/insert
 'skeleton-file-name               ; skeletonにより、プロンプトで補完入力したファイル名を挿入
...
(listify-set
 ...
 '(auto-insert-query nil)                      ; auto-insertでユーザーに尋ねない
 '(backup-directory-alist (("." "~/backup")))
 '(blink-cursor-mode nil)                      ; カーソルは点滅しない
 '(case-replace nil)                ; 置換時に大文字小文字を区別しない
 '(column-number-mode t)            ; 列番号を表示
 '(completion-ignored-extensions ; ファイル名の補完入力の対象外にする拡張子。diredで淡色表示される
 ...
 '(delete-by-moving-to-trash t)      ;  ファイルの削除で、ゴミ箱を使う
 '(delete-old-versions t) ; 古いバックアップファイルを自動的に削除する
 '(disabled-command-function nil) ; すべてのコマンドの使用制限を解除する
 '(display-buffer-alist (("^\\*shell\\*$" (display-buffer-same-window)) ("^\\*magit: .+" (display-buffer-same-window)))) ; バッファの表示方法
 '(electric-indent-mode nil) ; 改行時の自動インデントを無効に（Emacs24から、初期値が有効）
 '(enable-recursive-minibuffers t)      ; 再帰的にミニバッファを使う
 '(eval-expression-print-length nil)    ; evalした結果を全部表示する
 '(global-font-lock-mode t)       ; メジャーモードに合わせた色を付ける
 '(history-delete-duplicates t)   ; 重複する履歴は削除
 '(history-length t)              ; 履歴の数を無制限に
 '(indent-line-function indent-to-left-margin) ; インデント用のファンクション
 '(indent-tabs-mode nil)          ; タブをスペースに展開
 '(inhibit-startup-screen t)      ; 起動時の画面を表示しない
 '(initial-scratch-message nil)   ; *scratch* にメッセージを表示しない
 '(line-number-mode t)            ; 行番号を表示
 '(make-backup-files t)           ; バックアップファイルを作成する
 '(menu-bar-mode nil)             ; メニューバーを表示しない
 '(next-line-add-newlines nil) ; ファイル末尾での改行で、end of bufferエラーが発生しないように
 '(read-mail-command mew)      ; メールを読むときにmewを使う
 '(ring-bell-function ignore)  ; エラー時、なにもしない
 '(save-interprogram-paste-before-kill t) ; 他アプリのコピーバッファをkill-ringに保存する
 '(scroll-conservatively 1) ; 画面最下部で下向き、画面最上部で上向きにスクロールするとき、1行ずつスクロール
 ...
 '(use-dialog-box nil)                  ; ダイアログボックスは使わない
 '(user-mail-address "j8takagi@nifty.com") ; ChangeLogなどで用いるメールアドレスの設定
 '(version-control t)   ; バックアップファイルにバージョン番号を付ける
 '(yank-excluded-properties t)  ; ヤンクで、テキストプロパティは捨てる
 '(yank-pop-change-selection t) ; yank-popを有効にする
 )

(when window-system
  (listify-set
   '(default-frame-alist                ; デフォルトフレーム
      (
...
(listify-defaliases
 ...
 '(message-box message)                ; メッセージダイアログボックスは使わない
...
(with-eval-after-load 'view
  (listify-requires
   'set-view-mode                     ; read-onlyファイルをview-modeで開く
   'view-mode-vi-bindings             ; view-modeでviのキーバインド
   )
  (listify-set
   '(view-read-only t)
   )
...
(with-eval-after-load 'compile
  (listify-set
   '(compilation-scroll-output first-error) ; *compilation*バッファをスクロールして表示
   ))
...
(with-eval-after-load 'dired
  ...
  (listify-set
   '(dired-listing-switches "-alh")     ; lsのオプションにhを追加
   '(dired-recursive-copies always) ; diredでディレクトリーを再帰的にコピーするとき、確認しない
...
(with-eval-after-load 'lisp-mode
  ...
  (defun init-lisp-indent-line ()
    (set-variable 'indent-line-function 'lisp-indent-line)) ; インデントの設定
  (listify-set
...
(with-eval-after-load 'web-mode
  ...
  (listify-set
   '(web-mode-markup-indent-offset 0)     ; HTMLタグのインデントを0に
...
;; auto-mode-alistで、既存のモード設定を上書きする
...
;; ffap（find file at point）のキーバインド
(ffap-bindings)
